...
def write_alignment_model( model_file ):
    print "Writing model to file: %s" % model_file
    aligner.write_model(model_file)
    return

def write_aligned_training_data( aligned_file, nbest=1 ):
...
def align_sequences( aligner, part1, part2, s1in_sep="", s2in_sep=" ", nbest=1 ):
    seq1 = splitter( part1, s1in_sep )
    seq2 = splitter( part2, s2in_sep )
    aligner.entry2alignfstnoinit(seq1,seq2)
    results = aligner.write_alignment( aligner.num_fsas()-1, nbest )
    for result in results:
...
if __name__=="__main__":
    ...
    if not args.align==None:
        ...
        if not args.write_model==None:
            write_alignment_model( args.write_model )
        if not args.write_align==None:
            write_aligned_training_data( args.write_align, nbest=nbest )
    elif not args.model==None:
